...
from pbtools.pbsmvd.RWAlignWrapper import NWAligner,_NWAligner
...
class BarcodeAligner(object):
    def __init__(self, barcodeFile, maxSequenceLength = 64):
        ...
        self._aligner = NWAligner()
        
    def rc(self, s):
        return "".join([self.cMap[c] for c in s[::-1]])
    
    def align(self, seq1, seq2, penalty = -3, match = 2):
        if len(seq1) != 0 and len(seq2) != 0 :
            self._aligner.clear()
            self._aligner.addAlignPair(seq1, seq2)
            self._aligner.align( (match, penalty, penalty, penalty) )
            bestAlignment = self._aligner.getGlobalBestAlignment()
            bestScore = bestAlignment[2]
            #scores = bestAlignment[3]
            #print "t: %4d %4d %s" % bestAlignment[0]
            #print "q: %4d %4d %s" % bestAlignment[1]
            return bestScore
        else:
            return 0
